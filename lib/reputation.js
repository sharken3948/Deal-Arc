...
export async function addReview(toAddress, review) {
  const key      = `reviews:${toAddress.toLowerCase()}`;
  const existing = (await kv.get(key)) || [];
  await kv.set(key, [...existing, review]);
}

export async function getReviews(toAddress) {
  return (await kv.get(`reviews:${toAddress.toLowerCase()}`)) || [];
}
